package com.example.testc2;

import com.example.testc2.util.TestUtil;

import org.junit.Test;

import java.io.BufferedReader;
import java.io.Console;
import java.io.IOException;
import java.io.InputStreamReader;
import java.util.HashSet;
import java.util.LinkedList;
import java.util.Scanner;
import java.util.Set;

import static org.junit.Assert.*;

/**
 * Example local unit test, which will execute on the development machine (host).
 *
 * @see <a href="http://d.android.com/tools/testing">Testing documentation</a>
 *
 *
 * 算法与复杂度
 * https://www.zhihu.com/column/c_1233396414257823744
 *
 *
 */
public class ExampleUnitTest {

    @Test
    public void addition_isCorrect() {
        assertEquals(4, 2 + 2);
    }

    @Test
    public void testInPut() {
        Scanner s = new Scanner(System.in);
        System.out.println(s.nextLine());
    }


    @Test
    public void call1(){
//        int[] arr1 = {1,2,3,4,5};
//        test1(arr1,1);

        String value =  "0.5";
        String[] values = value.split("\\.");
        System.out.print(values.length);

    }




    Node head;
    Node tail;
    Node pre=null;

    public void test1(int[] arr, int step){

        //
        for (int i=0;i<arr.length;i++){
            Node t = new Node(arr[i]);
            if(i==0){
                head = t;
            }
            if(i==arr.length-1){
                tail = t;
            }
            if(pre!=null){
                pre.next = t;
                t.pre = pre;
            }
            pre = t;
        }


        for (int i=0;i<step;i++) {
            shift1Step();
        }

        Node n = head;
        while (n!=null){
            System.out.print(n.value+"|");
            n = n.next;
        }

    }


    private void shift1Step(){

        Node t = head;
        Node secondLast = tail.pre;
        secondLast.next = null;
        tail.pre = null;

        tail = secondLast;

        head = tail;
        head.pre=null;
        head.next = t;

    }






    static class Node {

        int value;

        Node next;
        Node pre;


        public Node(int value) {
            this.value = value;
        }

    }


    void test1(){
        //  08-19 20:48:35.708 D/UT_LOG_TAG( 3794): Id:2201   reStr:{"tbdm":{"foreign":{"clickUrl":"","exposeUrl":""}},"utReport":{"reportList":[{"args":{"zp_ad_strategy_group_id":4,"zp_ad_record_id":153791835,"zp_ad_business_type":2,"zp_content_id":5687,"zp_ad_subject_id":2581,"zp_ad_adgroup_id":102136,"zp_ad_plan_id":102053,"zpAdId":"0_19870102","zp_ad_scene_res_id":300001003,"zp_ad_activity_id":1002039,"zp_ad_scene_id":300,"zp_ad_device_hash":2056787173},"callBack":true,"clickId":"Button_Shoppingscreen_BannerPit","exposureId":"Expose_Shoppingscreen_BannerPit","page":"Page_ShoppingScreen"}]}}
    }




    @Test
    public void testStr(){
        assertEquals(TestUtil.convertChinese(1), "一");
        assertEquals(TestUtil.convertChinese(10), "十");
        assertEquals(TestUtil.convertChinese(11), "十一");
        assertEquals(TestUtil.convertChinese(12), "十二");
        assertEquals(TestUtil.convertChinese(20), "二十");
        assertEquals(TestUtil.convertChinese(21), "二十一");
        assertEquals(TestUtil.convertChinese(27), "二十七");
        assertEquals(TestUtil.convertChinese(107), "一百零七");
        assertEquals(TestUtil.convertChinese(117), "一百十七");
        assertEquals(TestUtil.convertChinese(127), "一百二十七");
        assertEquals(TestUtil.convertChinese(337), "三百三十七");
    }


}