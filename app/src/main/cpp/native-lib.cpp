...
JNIEXPORT jint JNICALL JNI_OnLoad(JavaVM *vm, void *reserved) {
     ...
     */
    __android_log_print(ANDROID_LOG_VERBOSE, "test1", "p1 address:%p",&p1);
...
extern "C"
JNIEXPORT jint JNICALL
Java_com_example_testc2_ffmpeg_TestFF1Activity_play(JNIEnv *env, jobject thiz, jstring url_,
                                              jobject surface) {
    const char *url = env->GetStringUTFChars(url_, 0);
//    注册所有的组件
...
//打开器
    ...
    if (0 == nativeWindow){
        LOGI("Couldn't get native window from surface.\n");
        return -1;
    }
//    三个容器
...
//
    ...
    if (0 > ANativeWindow_setBuffersGeometry(nativeWindow,width,height,WINDOW_FORMAT_RGBA_8888)){
        LOGI("Couldn't set buffers geometry.\n");
        ANativeWindow_release(nativeWindow);
        return -1;
    }
    LOGI("ANativeWindow_setBuffersGeometry成功\n");
//
